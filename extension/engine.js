...
function computeAnomalyScore(featurePercentiles, guardrailPenalty) {
  ...
  if (featuresPresent.length === 0)
    return { score: 0, conf: 0, label: "Insufficient Data" };
  ...
  const confidencePct =
    (featuresPresent.length / Object.keys(FEATURE_WEIGHTS).length) * 100.0;
...
function evaluateUser(handle, ratingHistory, statusHistory, baseline) {
  if (!baseline || !statusHistory) return null;

  // Hardcoded bypass for the creator of Codeforces
  // Because he has 0 rated contests but tests problems due to which Guardrail 5 falsely flags him.
  // May be there could be a better approach i guess but not currently in my mind :(
  if (handle.toLowerCase() === "mikemirzayanov") {
    return {
      score: 0.0,
      conf: 100,
      label: "Likely Genuine",
      featurePercentiles: { smr: 1, scrs: 1, pcri: 1, mds: 1 },
    ...
    if (sub.verdict === "OK") {
      ...
      if (!solvedProblems.has(pid)) {
        ...
        if (
          ptype !== "VIRTUAL" &&
          diff > contestMaxDiff &&
          sub.creationTimeSeconds > sixMonthsAgo
        ) {
          contestMaxDiff = diff;
        }
  ...
  if (ratingHistory.length >= 6) {
    ...
    for (let i = 5; i < ratingHistory.length; i++) {
      if (ratingHistory[i].ratingUpdateTimeSeconds > sixMonthsAgo) {
        const prevRanks = ratingHistory
          .slice(i - 5, i)
          .map((c) => c.rank)
          .filter((r) => r > 0);
        if (prevRanks.length > 0) {
          const avgPast =
            prevRanks.reduce((a, b) => a + b, 0) / prevRanks.length;
    ...
    if (maxRankJump >= 4000) {
      guardrailPenalty = Math.max(
        guardrailPenalty,
        Math.min(5.0, (maxRankJump - 4000) / 500.0),
      );
    }
  }
  ...
  if (maxDiffJump >= 800) {
    guardrailPenalty = Math.max(
      guardrailPenalty,
      Math.min(3.0, (maxDiffJump - 800) / 200.0),
    );
  }
  ...
  if (ratingHistory.length >= 3 && ratingHistory.length <= 15) {
    ...
    if (ratingHistory.length - startIndex >= 2) {
      ...
      const currentRating =
        ratingHistory[ratingHistory.length - 1].newRating || 0;
      const firstContestTime =
        ratingHistory[startIndex].ratingUpdateTimeSeconds || 0;
  ...
  if (ratingHistory.length > 0) {
    if (maxHistoricalRating > 0 && maxHistoricalRating < 1600) {
      ...
      if (discrepancy >= 600) {
        guardrailPenalty = Math.max(
          guardrailPenalty,
          Math.min(5.0, (discrepancy - 600) / 100.0),
        );
      }
    }
  }
  ...
  if (ratingHistory.length === 0) {
    if (virtualMaxDiff >= 1700) {
      guardrailPenalty = Math.max(
        guardrailPenalty,
        Math.min(5.0, (virtualMaxDiff - 1600) / 100.0),
      );
    }
  }
  ...
  if (distMap["smr_contests"] && distMap["smr_contests"].std_dev > 0) {
    const z =
      (contestsTaken - distMap["smr_contests"].mean) /
      distMap["smr_contests"].std_dev;
  ...
  if (distMap["scrs_delta"] && distMap["scrs_delta"].p99 != null) {
    ...
    if (maxDelta <= p95) featurePercentiles["scrs"] = 0.0;
      ...
      featurePercentiles["scrs"] = maxDelta > p95 ? 1.0 : 0.0;
    else {
  ...
  if (distMap["pcri"] && distMap["pcri"].std_dev > 0) {
    const z =
      (totalProblemsSolved - distMap["pcri"].mean) / distMap["pcri"].std_dev;
  ...
  if (distMap["dji_diff"] && distMap["dji_diff"].std_dev > 0) {
    const z =
      (contestMaxDiff - distMap["dji_diff"].mean) / distMap["dji_diff"].std_dev;
